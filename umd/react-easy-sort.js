(function (global, factory) {
  ...
  var useDrag = function useDrag(_a) {
    ...
    var handleClickStartTimerMoveRef = React__default['default'].useRef(null); // see https://twitter.com/ValentinHervieu/status/1324407814970920968
    ...
    var onMouseMove = React__default['default'].useCallback(function (e) {
      onDrag(getMousePoint(e)); // otherwise, we do the normal move logic
    }, [onDrag]);
    var moveStart = React__default['default'].useCallback(function (e) {
      var pointInWindow = getMousePoint(e);
      var point = getPointInContainer(pointInWindow, containerPositionRef.current); // if (Math.sqrt( Math.pow((oldPoint.x - point.x), 2) + Math.pow((oldPoint.y - point.y), 2)) > 50) {
      //   return;
      // }
    ...
    var onMouseDown = React__default['default'].useCallback(function (e) {
      ...
      var x = e.clientX;
      var y = e.clientY; // mark the next move as being the first one

      handleClickStartTimerMoveRef.current = window.setTimeout(function () {
        moveStart({
          clientX: x,
          clientY: y
        });
  ...
  var SortableList = function SortableList(_a) {
    var children = _a.children,
        ...
        rest = tslib.__rest(_a, ["children", "allowDrag", "onSortEnd", "draggedItemClassName", "as", "lockAxis", "customHolderRef"]); // this array contains the elements than can be sorted (wrapped inside SortableItem)
